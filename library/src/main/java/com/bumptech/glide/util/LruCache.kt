 ...
 *
 * @param size The maximum size of the cache, the units must match the units used in [     ][.getSize].
 */
open class LruCache<T, Y>(private val initialMaxSize: Long) {
    ...
    @get:Synchronized
    open var maxSize: Long = initialMaxSize

    /** Returns the sum of the sizes of all items in the cache.  */
    @get:Synchronized
    open var currentSize: Long = 0
     ...
     */
    protected open fun onItemEvicted(key: T, item: Y?) {
        // optional override
    }
    ...
    protected fun trimToSize(size: Long) {
        ...
        while (currentSize > size) {
            ...
            currentSize -= toRemove!!.size.toLong()
            val key = last.key
    ...
    @Synthetic
    internal class Entry<Y> @Synthetic constructor(val value: Y, val size: Int)
}